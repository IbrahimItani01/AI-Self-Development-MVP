...
import type { CheckInAnswers, GrowthPlan, Message, Student, UsageType } from "@/types";
import { AI_SYSTEM_INSTRUCTION, FOLLOW_UP_CLASSIFIER_INSTRUCTION } from "./prompts";
...
export function estimateUsageCost(inputTokens: number, outputTokens: number, usedModel = model): number {
...
async function callAI(input: {
  ...
  const inputTokens = completion.usage?.prompt_tokens ?? Math.ceil(input.prompt.length / 4);
...
function fallbackResponse(type: UsageType, prompt: string): string {
  if (type === "growth_plan") {
    return JSON.stringify({
      ...
      weeklyActions: ["Choose one priority each week", "Write one short reflection", "Ask for feedback when stuck"],
      reflectionPrompt: "What small action helped you move forward this week?",
      suggestedNextStep: "Pick one 15-minute action you can complete before tomorrow.",
    });
  }
  if (type === "classification") {
    return JSON.stringify({
      ...
      summary: "The reflection appears suitable for normal student development support.",
...
export async function generateGrowthPlan(student: Student, onboardingAnswers: Record<string, unknown>): Promise<Omit<GrowthPlan, "id" | "createdAt" | "updatedAt">> {
...
export async function summarizeConversation(student: Student, recentMessages: Message[]): Promise<string> {
...
Do not include sensitive details. Focus on goals, progress, blockers, and next steps.`;
  const result = await callAI({
    organizationId: student.organizationId,
    studentId: student.id,
    type: "summary",
...
export async function generateCheckInSummary(student: Student, checkInAnswers: CheckInAnswers): Promise<{ summary: string; suggestedNextStep: string }> {
  const prompt = `Create a concise weekly check-in summary and next step.
Student: ${student.displayName}
Answers: ${JSON.stringify(checkInAnswers)}
Return two short paragraphs: Summary and Suggested next step.`;
  const result = await callAI({
    organizationId: student.organizationId,
    studentId: student.id,
    type: "check_in",
  ...
  const [summaryPart, nextPart] = result.text.split(/Suggested next step:|Next step:/i);
  return {
...
export async function classifyFollowUpNeed(student: Student, textOrSummary: string): Promise<FollowUpClassification> {
  const prompt = `Student: ${student.displayName}
Focus area: ${student.selectedFocusArea ?? "Not set"}
Text to classify:
${textOrSummary}`;
