...
export function Button({ className, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button className={cn(styles, "bg-primary text-white hover:bg-primaryDark", className)} {...props} />;
}

export function SecondaryButton({ className, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button className={cn(styles, "border border-ink/10 bg-surface text-ink hover:bg-canvas", className)} {...props} />;
}
